classdef Device < handle
    ...
    methods
        ...
        function Jout = source(self)
            ...
            spectra = gausswin(length(self.k),2);%%;hanning(length(self.k)); ones(length(self.k),1); 
...
%             Fout(:,1) = (Fout(:,1),round(length(self.k)/realpn*2);
%             Fout(:,2) = (Fout(:,2),round(length(self.k)/realpn*2);
        ...
        function Fout = dual_balanced_polarizaiton_diversity_detector_true(self,Jref,Jsample)
...
%             Fout(:,2) = (Fout(:,2),round(length(self.k)/realpn*2);
            ...
            Fout = Fout;%+0.0000000000005*randn(size(Fout)); %0.0001
            
        end

        function [Fout,snr] = dual_balanced_polarizaiton_diversity_detector_with_noise(self,Jref,Jsample,noise_factor)
...
%             Fout(:,2) = (Fout(:,2),round(length(self.k)/realpn*2);
            ...
            signal = sum(Fout(:).^2);
            Fout = Fout+noise_factor*randn(size(Fout)); %0.0001
            noise = sum((noise_factor*randn(size(Fout))).^2,"all");
